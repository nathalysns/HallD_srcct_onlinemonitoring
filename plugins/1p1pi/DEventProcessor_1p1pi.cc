...
#include "DEventProcessor_1p1pi.h"
...
extern "C"{
  void InitPlugin(JApplication *app){
    InitJANAPlugin(app);
    app->AddProcessor(new DEventProcessor_1p1pi());
  }
} // "C"

//define static local variable //declared in header file
thread_local DTreeFillData DEventProcessor_1p1pi::dTreeFillData;

//------------------
// init
//------------------
jerror_t DEventProcessor_1p1pi::init(void)
{
 
  //TTREE INTERFACE
  //MUST DELETE WHEN FINISHED: OR ELSE DATA WON'T BE SAVED!!!
  string treeName = "tree_1p1pi";
  string treeFile = "tree_1p1pi.root";
  gPARMS->SetDefaultParameter("SRC_1P1PI:TREENAME", treeName);
  gPARMS->SetDefaultParameter("SRC_1P1PI:TREEFILE", treeFile);
...
// brun
//------------------
jerror_t DEventProcessor_1p1pi::brun(JEventLoop *eventLoop, int32_t runnumber)
{
...
// evnt
//------------------
jerror_t DEventProcessor_1p1pi::evnt(JEventLoop *loop, uint64_t eventnumber)
{
  ...
  if ((ch_tracks.size() != 2) || (showers.size() != 0))
    return NOERROR;
...
// erun
//------------------
jerror_t DEventProcessor_1p1pi::erun(void)
{
...
// fini
//------------------
jerror_t DEventProcessor_1p1pi::fini(void)
{
...
void DEventProcessor_1p1pi::GetHypotheses(vector<const DChargedTrack *> &tracks,
